import type { components, operations } from "../generated/schema";

export type StateSnapshot = components["schemas"]["StateSnapshot"];
export type StateScope = components["schemas"]["StateScope"];
export type DeviceSet = components["schemas"]["DeviceSet"];
export type DeviceInfo = components["schemas"]["DeviceInfo"];
export type DeviceSettings = components["schemas"]["DeviceSettings"];
export type Capabilities = components["schemas"]["Capabilities"];
export type DeviceProfile = components["schemas"]["DeviceProfile"];
export type StreamScope = components["schemas"]["StreamScope"];
export type StreamSettings = components["schemas"]["StreamSettings"];
export type Tuning = components["schemas"]["Tuning"];
export type Duplex = components["schemas"]["Duplex"];
export type GainStage = components["schemas"]["GainStage"];
export type GainKind = components["schemas"]["GainKind"];
export type Agc = components["schemas"]["Agc"];
export type AgcSetting = components["schemas"]["AgcSetting"];
export type BandwidthSetting = components["schemas"]["BandwidthSetting"];
export type Range = components["schemas"]["Range"];
export type ExtraSetting = components["schemas"]["ExtraSetting"];
export type DevicesResponse = components["schemas"]["DevicesResponse"];
export type ChannelInfo = components["schemas"]["ChannelInfo"];
export type ChannelLevel = components["schemas"]["ChannelLevel"];
export type OccupancyReport = components["schemas"]["OccupancyReport"];
export type OccupancyBucket = components["schemas"]["OccupancyBucket"];
export type IonosondeReport = components["schemas"]["IonosondeReport"];
export type IonosondeStation = components["schemas"]["IonosondeStation"];
export type PropagationNode = components["schemas"]["PropagationNode"];
export type ChannelSettings = components["schemas"]["ChannelSettings"];
export type ChannelParams = components["schemas"]["ChannelParams"];
export type ChannelDescriptor = components["schemas"]["ChannelDescriptor"];
export type DecoderFamily = components["schemas"]["DecoderFamily"];
export type ParamLimit = components["schemas"]["ParamLimit"];
export type Squelch = components["schemas"]["Squelch"];
export type EventFilterNode = components["schemas"]["EventFilterNode"];
export type SpectrumMonitorNode = components["schemas"]["SpectrumMonitorNode"];
export type FilterMode = components["schemas"]["FilterMode"];
export type EventFacet = components["schemas"]["EventFacet"];
export type EventKindFacets = components["schemas"]["EventKindFacets"];
export type AudioProcessing = components["schemas"]["AudioProcessing"];
export type AudioAgcMode = components["schemas"]["AudioAgcMode"];
export type AudioFilterSettings = components["schemas"]["AudioFilterSettings"];
export type DenoiseSettings = components["schemas"]["DenoiseSettings"];
export type DenoiseMode = components["schemas"]["DenoiseMode"];
export type AudioFxNode = components["schemas"]["AudioFxNode"];
export type NoiseBlankerSettings = components["schemas"]["NoiseBlankerSettings"];
export type NotchSettings = components["schemas"]["NotchSettings"];
export type ChannelTypesResponse = components["schemas"]["ChannelTypesResponse"];
export type PresetInfo = components["schemas"]["PresetInfo"];
export type Bookmark = components["schemas"]["Bookmark"];
export type CreateBookmarkRequest = components["schemas"]["CreateBookmarkRequest"];
export type RecordingStatus = components["schemas"]["RecordingStatus"];
export type AudioRecordingStatus = components["schemas"]["AudioRecordingStatus"];
export type AudioRecordingInfo = components["schemas"]["AudioRecordingInfo"];
export type AudioRecordingsResponse = components["schemas"]["AudioRecordingsResponse"];
export type ClickRemovalSettings = components["schemas"]["ClickRemovalSettings"];
export type NetworkExportAction = components["schemas"]["NetworkExportAction"];
export type NetworkExportSettings = components["schemas"]["NetworkExportSettings"];
export type NetworkExportStatus = components["schemas"]["NetworkExportStatus"];
export type TimeMachineAction = components["schemas"]["TimeMachineAction"];
export type TimeMachineNode = components["schemas"]["TimeMachineNode"];
export type TimeMachineStatus = components["schemas"]["TimeMachineStatus"];
export type NetworkSampleFormat = components["schemas"]["NetworkSampleFormat"];
export type NetworkTransport = components["schemas"]["NetworkTransport"];
export type EventOutputTarget = components["schemas"]["EventOutputTarget"];
export type PlaybackStatus = components["schemas"]["PlaybackStatus"];
export type PlaybackAction = components["schemas"]["PlaybackAction"];
export type RecordingInfo = components["schemas"]["RecordingInfo"];
export type RecordingAnnotation = components["schemas"]["RecordingAnnotation"];
export type RecordingsResponse = components["schemas"]["RecordingsResponse"];
export type VoiceCall = components["schemas"]["VoiceCall"];
export type DvTrunkProtocol = components["schemas"]["DvTrunkProtocol"];
export type TrunkSystemStatus = components["schemas"]["TrunkSystemStatus"];
export type TrunkFollower = components["schemas"]["TrunkFollower"];
export type TrunkProblem = components["schemas"]["TrunkProblem"];
export type TrunkChannel = components["schemas"]["TrunkChannel"];
export type TrunkChannelSource = components["schemas"]["TrunkChannelSource"];
export type TrunkProbe = components["schemas"]["TrunkProbe"];
export type VoiceCallsResponse = components["schemas"]["VoiceCallsResponse"];
export type CapturedImage = components["schemas"]["CapturedImage"];
export type CapturedImagesResponse = components["schemas"]["CapturedImagesResponse"];
export type SstvMode = components["schemas"]["SstvMode"];
export type SstvPicture = components["schemas"]["SstvPicture"];
export type RecordingFormat = components["schemas"]["RecordingFormat"];
export type DecodedRecord = components["schemas"]["DecodedRecord"];
export type DecoderEvent = components["schemas"]["DecoderEvent"];
export type DecoderLogEntry = components["schemas"]["DecoderLogEntry"];
export type DecoderLogResponse = components["schemas"]["DecoderLogResponse"];
export type DeletedCount = components["schemas"]["DeletedCount"];
export type ExportFormat = components["schemas"]["ExportFormat"];
export type AdsbMessage = components["schemas"]["AdsbMessage"];
export type AisMessage = components["schemas"]["AisMessage"];
export type AprsPacket = components["schemas"]["AprsPacket"];
export type PocsagMessage = components["schemas"]["PocsagMessage"];
export type FlexMessage = components["schemas"]["FlexMessage"];
export type ErmesMessage = components["schemas"]["ErmesMessage"];
export type RdsUpdate = components["schemas"]["RdsUpdate"];
export type RttyText = components["schemas"]["RttyText"];
export type MorseText = components["schemas"]["MorseText"];
export type CwSkimmerSpot = components["schemas"]["CwSkimmerSpot"];
export type RadioClockFrame = components["schemas"]["RadioClockFrame"];
export type GnssFrame = components["schemas"]["GnssFrame"];
export type VorReading = components["schemas"]["VorReading"];
export type IlsReading = components["schemas"]["IlsReading"];
export type DataLinkMessage = components["schemas"]["DataLinkMessage"];
export type DectFrame = components["schemas"]["DectFrame"];
export type DvFrame = components["schemas"]["DvFrame"];
export type IdentReport = components["schemas"]["IdentReport"];
export type IdentSignal = components["schemas"]["IdentSignal"];
export type ProtocolMatch = components["schemas"]["ProtocolMatch"];
export type Modulation = components["schemas"]["Modulation"];
export type ServerEvent = components["schemas"]["ServerEvent"];
export type StreamKind = components["schemas"]["StreamKind"];
export type ClientCommand = components["schemas"]["ClientCommand"];
export type ApiError = components["schemas"]["ApiError"];
export type ErrorCode = components["schemas"]["ErrorCode"];
export type AuthInfo = components["schemas"]["AuthInfo"];
export type ClientsResponse = components["schemas"]["ClientsResponse"];
export type TemplateInfo = components["schemas"]["TemplateInfo"];
export type TemplatesResponse = components["schemas"]["TemplatesResponse"];
export type ScanSettings = components["schemas"]["ScanSettings"];
export type ScanRange = components["schemas"]["ScanRange"];
export type ScannerStatus = components["schemas"]["ScannerStatus"];
export type ScanState = components["schemas"]["ScanState"];
export type ScanMode = components["schemas"]["ScanMode"];
export type HuntSettings = components["schemas"]["HuntSettings"];
export type HuntStatus = components["schemas"]["HuntStatus"];
export type SatelliteNode = components["schemas"]["SatelliteNode"];
export type SatelliteStatus = components["schemas"]["SatelliteStatus"];
export type SatellitePass = components["schemas"]["SatellitePass"];
export type CatalogSatellite = components["schemas"]["CatalogSatellite"];
export type SatelliteCatalogResponse = components["schemas"]["SatelliteCatalogResponse"];
export type Transmitter = components["schemas"]["Transmitter"];
export type TransmittersResponse = components["schemas"]["TransmittersResponse"];
export type WorkspaceInfo = components["schemas"]["WorkspaceInfo"];
export type WorkspacesResponse = components["schemas"]["WorkspacesResponse"];
export type WorkspaceDetail = components["schemas"]["WorkspaceDetail"];
export type WorkspaceSnapshot = components["schemas"]["WorkspaceSnapshot"];
export type WorkspaceSettings = components["schemas"]["WorkspaceSettings"];
export type WorkspaceExport = components["schemas"]["WorkspaceExport"];
export type PatchGraph = components["schemas"]["PatchGraph"];
export type PatchNode = components["schemas"]["PatchNode"];
export type Position = components["schemas"]["Position"];
export type NodeBody = components["schemas"]["NodeBody"];
export type NodeKind = NodeBody["kind"];
export type DmrTrunkProtocol = components["schemas"]["DmrTrunkProtocol"];
export type DmrDiscovery = components["schemas"]["DmrDiscovery"];
export type DmrSearchRange = components["schemas"]["DmrSearchRange"];
export type DmrChannelEntry = components["schemas"]["DmrChannelEntry"];
export type NodeCategory = components["schemas"]["NodeCategory"];
export type PatchEdge = components["schemas"]["PatchEdge"];
export type PortRef = components["schemas"]["PortRef"];
export type PortSpec = components["schemas"]["PortSpec"];
export type PortType = components["schemas"]["PortType"];
export type PositionFix = components["schemas"]["PositionFix"];
export type PositionSource = components["schemas"]["PositionSource"];
export type NmeaDeviceInfo = components["schemas"]["NmeaDeviceInfo"];
export type NmeaDevicesResponse = components["schemas"]["NmeaDevicesResponse"];
export type PortDirection = components["schemas"]["PortDirection"];
export type PortCondition = components["schemas"]["PortCondition"];
export type PatchCatalog = components["schemas"]["PatchCatalog"];
export type NodeTypeInfo = components["schemas"]["NodeTypeInfo"];
export type DeviceRef = components["schemas"]["DeviceRef"];
export type RackLayout = components["schemas"]["RackLayout"];
export type RackSlot = components["schemas"]["RackSlot"];
export type PatchApplyReport = components["schemas"]["PatchApplyReport"];
export type PatchBinding = components["schemas"]["PatchBinding"];
export type BandPlan = components["schemas"]["BandPlan"];
export type BandLane = components["schemas"]["BandLane"];
export type BandBlock = components["schemas"]["BandBlock"];
export type BandAllocation = components["schemas"]["BandAllocation"];
export type BandLayerInfo = components["schemas"]["BandLayerInfo"];
export type BandRegion = components["schemas"]["BandRegion"];
export type BandRegionsResponse = components["schemas"]["BandRegionsResponse"];
export type BandService = components["schemas"]["BandService"];
export type AboutResponse = components["schemas"]["AboutResponse"];
export type Attribution = components["schemas"]["Attribution"];
export type ComponentSource = components["schemas"]["ComponentSource"];
export type LicenseTextResponse = components["schemas"]["LicenseTextResponse"];
export type ToolDescriptor = components["schemas"]["ToolDescriptor"];
export type ToolCategory = components["schemas"]["ToolCategory"];
export type ToolsResponse = components["schemas"]["ToolsResponse"];

export type Codeplug = components["schemas"]["Codeplug"];
export type CodeplugCounts = components["schemas"]["CodeplugCounts"];
export type CodeplugChannel = components["schemas"]["Channel"];
export type CodeplugContact = components["schemas"]["Contact"];
export type CodeplugGroupList = components["schemas"]["GroupList"];
export type CodeplugZone = components["schemas"]["Zone"];
export type CodeplugScanList = components["schemas"]["ScanList"];
export type CodeplugRadioId = components["schemas"]["RadioId"];
export type ChannelKind = components["schemas"]["ChannelKind"];
export type CodeplugPower = components["schemas"]["Power"];
export type Tone = components["schemas"]["Tone"];
export type ConversionReport = components["schemas"]["ConversionReport"];
export type ConversionIssue = components["schemas"]["ConversionIssue"];
export type IssueSeverity = components["schemas"]["IssueSeverity"];
export type RadioModelDescriptor = components["schemas"]["RadioModelDescriptor"];
export type RadioModelsResponse = components["schemas"]["RadioModelsResponse"];
export type RadioLimits = components["schemas"]["RadioLimits"];
export type CpsPort = components["schemas"]["CpsPort"];
export type CpsPortsResponse = components["schemas"]["CpsPortsResponse"];
export type CpsLibraryResponse = components["schemas"]["CpsLibraryResponse"];
export type CpsUser = components["schemas"]["CpsUser"];
export type CpsUserRequest = components["schemas"]["CpsUserRequest"];
export type CpsDevice = components["schemas"]["CpsDevice"];
export type CpsDeviceRequest = components["schemas"]["CpsDeviceRequest"];
export type CpsCodeplugInfo = components["schemas"]["CpsCodeplugInfo"];
export type CpsCodeplugDetail = components["schemas"]["CpsCodeplugDetail"];
export type CpsCodeplugRequest = components["schemas"]["CpsCodeplugRequest"];
export type CpsConvertRequest = components["schemas"]["CpsConvertRequest"];
export type CpsConvertResponse = components["schemas"]["CpsConvertResponse"];
export type CpsMergeRequest = components["schemas"]["CpsMergeRequest"];
export type MergeMode = components["schemas"]["MergeMode"];
export type MergePart = components["schemas"]["MergePart"];
export type CpsJob = components["schemas"]["CpsJob"];
export type CpsJobState = components["schemas"]["CpsJobState"];
export type CpsJobsResponse = components["schemas"]["CpsJobsResponse"];
export type CpsReadRequest = components["schemas"]["CpsReadRequest"];
export type CpsWriteRequest = components["schemas"]["CpsWriteRequest"];
export type RadioIdent = components["schemas"]["RadioIdent"];
export type ToolRequest = components["schemas"]["ToolRequest"];
export type ToolResponse = components["schemas"]["ToolResponse"];
export type ToolId = ToolRequest["tool"];
export type AntennaRequest = components["schemas"]["AntennaRequest"];
export type AntennaReport = components["schemas"]["AntennaReport"];
export type AntennaDesign = components["schemas"]["AntennaDesign"];
export type AntennaPart = components["schemas"]["AntennaPart"];
export type AntennaGeometry = components["schemas"]["AntennaGeometry"];
export type AntennaPoint = components["schemas"]["AntennaPoint"];
export type AntennaSegment = components["schemas"]["AntennaSegment"];
export type AntennaSegmentRole = components["schemas"]["AntennaSegmentRole"];
export type NanoVnaRequest = components["schemas"]["NanoVnaRequest"];
export type NanoVnaSweepRequest = components["schemas"]["NanoVnaSweepRequest"];
export type NanoVnaResult = components["schemas"]["NanoVnaResult"];
export type NanoVnaDevice = components["schemas"]["NanoVnaDevice"];
export type NanoVnaSweep = components["schemas"]["NanoVnaSweep"];
export type NanoVnaPoint = components["schemas"]["NanoVnaPoint"];
export type NanoVnaComplex = components["schemas"]["NanoVnaComplex"];
export type NanoVnaDeviceReport = components["schemas"]["NanoVnaDeviceReport"];
export type NanoVnaSweepState = components["schemas"]["NanoVnaSweepState"];
export type NanoVnaCalibration = components["schemas"]["NanoVnaCalibration"];
export type NanoVnaStandard = components["schemas"]["NanoVnaStandard"];
export type NanoVnaMatch = components["schemas"]["NanoVnaMatch"];
export type AntennaDesignType = AntennaDesign["type"];
export type AntennaDesignOf<T extends AntennaDesignType> = Extract<AntennaDesign, { type: T }>;
export type DoctorReport = components["schemas"]["DoctorReport"];
export type DiagnosticsReport = components["schemas"]["DiagnosticsReport"];
export type LogLine = components["schemas"]["LogLine"];
export type LogLevel = components["schemas"]["LogLevel"];
export type Coherence = components["schemas"]["Coherence"];
export type ArrayNode = components["schemas"]["ArrayNode"];
export type CombineMode = components["schemas"]["CombineMode"];
export type CombinerParams = components["schemas"]["CombinerParams"];
export type StitchMode = components["schemas"]["StitchMode"];
export type StitchParams = components["schemas"]["StitchParams"];
export type ArrayGeometry = components["schemas"]["ArrayGeometry"];
export type ArrayElement = components["schemas"]["ArrayElement"];
export type DfParams = components["schemas"]["DfParams"];
export type DfAlgorithm = components["schemas"]["DfAlgorithm"];
export type DfReading = components["schemas"]["DfReading"];
export type CalParams = components["schemas"]["CalParams"];
export type CalSource = components["schemas"]["CalSource"];
export type CalState = components["schemas"]["CalState"];
export type LaneCal = components["schemas"]["LaneCal"];
export type PassiveRadarParams = components["schemas"]["PassiveRadarParams"];
export type EcaParams = components["schemas"]["EcaParams"];
export type CfarParams = components["schemas"]["CfarParams"];
export type Illuminator = components["schemas"]["Illuminator"];
export type RadarDetection = components["schemas"]["RadarDetection"];
export type DfEstimate = components["schemas"]["DfEstimate"];
export type DfGuidance = components["schemas"]["DfGuidance"];
export type GuidanceMode = components["schemas"]["GuidanceMode"];
export type NavTarget = components["schemas"]["NavTarget"];
export type NavTargetKind = components["schemas"]["NavTargetKind"];
export type DfFusionState = components["schemas"]["DfFusionState"];
export type DfStation = components["schemas"]["DfStation"];
export type Route = components["schemas"]["Route"];
export type RoutePoint = components["schemas"]["RoutePoint"];
export type RouteRequest = components["schemas"]["RouteRequest"];
export type Maneuver = components["schemas"]["Maneuver"];
export type ManeuverKind = components["schemas"]["ManeuverKind"];
export type DoctorCheck = components["schemas"]["DoctorCheck"];
export type CheckStatus = components["schemas"]["CheckStatus"];

export type DecoderLogFilter = NonNullable<operations["list_decoder_log"]["parameters"]["query"]>;

export type DecoderKind = DecoderEvent["kind"];
export type DecoderEventOf<K extends DecoderKind> = Extract<DecoderEvent, { kind: K }>;
export type DecodedRecordOf<K extends DecoderKind> = Omit<DecodedRecord, "event"> & {
  event: DecoderEventOf<K>;
};

export type NodeBodyOf<K extends NodeKind> = Extract<NodeBody, { kind: K }>;
export type PatchNodeOf<K extends NodeKind> = Omit<PatchNode, keyof NodeBody> & NodeBodyOf<K>;
