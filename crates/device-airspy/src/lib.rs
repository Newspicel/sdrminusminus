...
impl AirspyDriver {
    ...
    pub fn open_device(&self, info: &DeviceInfo) -> Result<AirspyDevice, DeviceError> {
        let device = match (key_serial(&info.key), key_location(&info.key)) {
            (Some(serial), _) => Airspy::open_serial(serial),
            ...
            (None, Some((bus, address))) => Airspy::open_at(bus, address),
            (None, None) => return Err(DeviceError::NotFound(info.id())),
        }
        .map_err(map_err)?;
        Ok(AirspyDevice::new(device))
    }
}

impl DeviceDriver for AirspyDriver {
    ...
    fn probe(&self) -> Vec<DeviceInfo> {
        match Airspy::list() {
            ...
            Err(e) => {
                tracing::warn!("airspy enumerate failed: {e}");
                Vec::new()
            }
        }
    }
...
impl AirspyRadio {
    fn lock(&self) -> MutexGuard<'_, Airspy> {
        lock(&self.device)
    }
}

impl CaptureRadio for AirspyRadio {
    ...
    fn disarm(&self) {
        if let Err(e) = self.lock().set_mode_off() {
            tracing::debug!("airspy stop failed: {e}");
        }
    }
}

pub struct AirspyDevice {
    radio: Arc<AirspyRadio>,
    capabilities: Capabilities,
    settings: DeviceSettings,
    duplex: Arc<Mutex<DuplexState>>,
    capture: Capture<AirspyRadio>,
}

impl AirspyDevice {
    fn new(device: Airspy) -> Self {
        let capabilities = caps::capabilities(device.sample_rates());
        let settings = caps::settings(device.config());
        tracing::debug!(firmware = device.version(), serial = ?device.serial(), "airspy ready");
        Self {
            radio: Arc::new(AirspyRadio {
...
impl SdrDevice for AirspyDevice {
    ...
    fn rx_start(&mut self, sinks: Vec<RxSink>) -> Result<(), DeviceError> {
        ...
        let started = self.capture.start(
            self.radio.clone(),
            convert::AirspyConverter::new(RX_TRANSFER_SIZE / 2),
            sink,
            CaptureConfig::new("sdrmm-airspy-rx", DRIVER_ID)
...
mod tests {
    ...
    fn descriptor(serial: Option<u64>) -> DeviceDescriptor {
        DeviceDescriptor {
            description: "Airspy",
            serial,
            product_string: Some("AIRSPY".to_string()),
            bus: "020".to_string(),
            address: 7,
        }
    }
    ...
    fn a_radio_with_a_serial_is_keyed_by_it() {
        ...
        assert_eq!(info.driver, "airspy");
        assert_eq!(info.key, "004400002e19a5b3");
        assert_eq!(info.label, "AIRSPY 004400002e19a5b3");
