...
/// `gap_us` of silence before each frame and after the last.
#[must_use]
pub fn transmission(frames: &[Vec<u8>], gap_us: f64, level: f32, rate: f64) -> Vec<Complex<f32>> {
    ...
    let silence = std::iter::repeat_n(Complex::new(0.0, 0.0), gap);
    let mut iq: Vec<Complex<f32>> = Vec::new();
    for frame in frames {
        iq.extend(silence.clone());
        let chips = half_chips(frame);
        let len = (chips.len() as f64 * 0.5 * samples_per_us).round() as usize;
        iq.extend((0..len).map(|k| {
            let chip = (k as f64 * 2.0 / samples_per_us) as usize;
            if chips.get(chip).copied().unwrap_or(false) {
