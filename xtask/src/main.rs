...
/// The Windows graph carries `windows-rs` twice — Tauri pins 0.61, wgpu 0.62 — and
/// gpu-allocator accepts either (`>=0.53, <=0.62`). When the resolver hands gpu-allocator
/// Tauri's copy, the `ID3D12Heap` it allocates is a different type from the one wgpu-hal's dx12
/// backend hands to `CreatePlacedResource`, and every Windows artifact fails to compile. No
/// Linux or macOS build ever resolves that pairing, so a `cargo update` can introduce it with
/// the whole gate green — which is what shipped two broken release matrices.
fn check_windows_rs_alignment(root: &Path) -> Result<()> {
    ...
    // Either crate leaving the tree retires the invariant with it.
...
/// The version `package` resolves `dependency` to, as the lock records it: a name alone when the
/// tree holds one version of it, `name version` when it holds several.
...
"#;
    ...
    fn a_prefix_of_another_crate_name_is_not_a_match() {
        // `windows-core 0.62.2` sits next to `windows 0.62.2` in wgpu-hal's list.
        assert_eq!(
