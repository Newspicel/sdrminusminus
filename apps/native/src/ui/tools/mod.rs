...
#[derive(Clone, Copy)]
pub struct Tools(pub RwSignal<Option<String>>);

pub fn provide() -> RwSignal<Option<String>> {
    let open = RwSignal::new(None);
    provide_context(Tools(open));
    open
}

pub fn open(store: Store, tool: Option<&str>) {
    match use_context::<Tools>() {
        Some(Tools(open)) => open.set(Some(tool.unwrap_or_default().to_owned())),
        None => store.say("Tools are not ready yet"),
    }
}
