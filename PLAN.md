...
│   ├── usb-stream/     # bulk-IN transport shared by the native backends: transfer queue,
│   │                   # USB error policy, mock seam. Bytes only, no device knowledge.
│   ├── device/         # SdrDevice traits, capability model, device registry,
│   │                   # shared stream-restart policy (pure).
...
│   │                   # src/driver/ is the HackRF radio, RX + TX, no wire types (§17).
│   │                   # TX stops at this crate's API — nothing above reaches it (§12a).
...
**What exists today, and where it stops.** `device-hackrf`'s driver has a working bulk-OUT
transmit path — burst queueing, the firmware's end-of-burst marker, half-duplex arbitration and
transmit VGA control — because the radio has it and a driver that omitted half its hardware
would have to be rewritten later. It stops at the crate's own API: `SdrDevice` has no transmit
method, `Capabilities` reports `tx_capable: false`, and nothing in `engine`, `server` or the web
UI can reach it. The transmit VGA is written to 0 dB when the device is opened, so a mode change
alone cannot make the radio radiate at drive. Everything below is what is still unbuilt.
