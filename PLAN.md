...
│   ├── usb-stream/     # bulk transport shared by the native backends, both directions:
│   │                   # transfer queues, USB error policy, mock seam (`test-util`).
│   │                   # Bytes only, no device knowledge.
│   ├── device/         # SdrDevice/TxStream traits, capability model, device registry, and
│   │                   # everything a backend would otherwise re-write: duplex arbitration
│   │                   # (§6), the capture thread + tier-1 restart supervisor, the 8-bit
│   │                   # sample-table converter, the restart policy. Pure; the `usb` feature
│   │                   # adds the CaptureStream impl for the transport above.
│   ├── device-soapy/   # SoapySDR backend (default feature).
│   ├── device-rtlsdr/  # native RTL-SDR backend (feature "rtl-native"). Owns its driver:
│   │                   # src/driver/ is the RTL2832U + R82xx radio, no wire types (§17).
│   ├── device-hackrf/  # native HackRF backend (feature "hackrf-native"). Owns its driver:
│   │                   # src/driver/ is the HackRF radio, RX + TX, no wire types and no
│   │                   # arbitration (§17). TX is implemented and gated: nothing in engine,
│   │                   # server or web reaches `tx_start`, `tx_capable` is false (§12a).
...
trait SdrDevice: Send {
    ...
    fn rx_stop(&mut self);
    fn duplex(&self) -> Duplex { Duplex::RxOnly }         // RxOnly | TxOnly | Half | Full
    fn tx_start(&mut self) -> Result<Box<dyn TxStream>> { Err(Unsupported) }
}
```
...
| Single-maintainer native-driver crates | resolved by taking the drivers in-tree: each native backend now owns its radio under `src/driver/`, on the shared `crates/usb-stream` transport. The trigger was not maintenance but correctness — each crate hand-rolled its own USB transfer-error policy and both were wrong (§18). This is the project's stated exception to "always newest versions": there is no upstream version to track any more, because the code is ours |
...
| Native backends (M5) | **Superseded — both are now vendored, see the row below.** As shipped at M5: `rs-rtl` 0.4.2 (RTL-SDR) + `hackrf-nusb` 0.3 (HackRF), both pure Rust over `nusb` 0.2 — so a release artifact links no C library and launches with nothing installed (§15). Rejected: `rtl-sdr-rs`/`seify-rtlsdr`/`rtlsdr_mt` (rusb/libusb or librtlsdr — a C dependency), `waverave-hackrf` (stale, pulls nusb 0.1 *and* 0.2), `seify` (wraps hackrf-nusb and duplicates our own device abstraction). Known gaps, documented rather than faked: no PPM or direct-sampling through rs-rtl's public API, no independent baseband-filter bandwidth or hardware sweep through hackrf-nusb's. Registered above Soapy in the serial merge |
| RTL-SDR driver, re-evaluated (post-M5) | Re-checked on technical merit alone after §17 accepted GPL dependencies, so the original licence-based rejection of `rtlsdr-pure` no longer decides anything. **Outcome: keep `rs-rtl` 0.4.2** — it is the only candidate whose read path keeps a queue of USB transfers in flight (`NUM_TRANSFERS = 15`, `Endpoint::submit` × 15 then `wait_next_complete`). `rtlsdr-pure` 0.2.3's sole read API is `read_bytes(&self, len)` over a `BulkReader` that submits **one** buffer and awaits it (`rtl2832.rs:880-881`), leaving the endpoint empty between transfers. **Measured on the NESDR SMArt v5 rather than argued** (RTL2832U test mode emits an 8-bit counter ramp, so every lost byte is a countable discontinuity): idle, rtlsdr-pure is fine — 8 ramp breaks in 131 MB at 2.4 MS/s, full rate delivered, so the "starves the FIFO" claim is *false as stated for an idle machine*. Under 16 spinning threads it loses ~2.2% of the stream (2.048 MS/s: 4.01 of 4.10 MB/s, 241 breaks; 2.4 MS/s: 4.69 of 4.80 MB/s, 254 breaks) — roughly one discontinuity every 0.1 s, which is fatal for decoder framing and, on an odd-length gap, permanently swaps I/Q against `convert.rs`'s cross-block alignment. rs-rtl on the same machine, load, rates and byte count delivered **100.0%** with 0 dropped chunks. The 15-transfer queue is exactly the margin that buys, and sdr-- runs a full DSP pipeline on those cores. rtlsdr-pure would also *regress* two working controls: direct sampling is absent entirely, and the bias tee is unreachable (`set_gpio_bit` is `pub(crate)`), so its PPM support does not pay for it. `rtl-sdr-rs` 0.3.3 / `seify-rtlsdr` 0.0.4 expose the full control set (PPM, direct sampling, offset tuning) but only `read_sync(&self, buf)` over a single blocking `rusb::read_bulk` — same starvation, plus the C dependency. Both remaining gaps are ours to close, not a reason to switch. The transient-stall teardown is a defect in rs-rtl's error accounting and is fixable entirely on our side (see PROGRESS "Known gaps"). PPM and direct sampling need upstream setters, *not* a register escape hatch: `rs_rtl::device::Device` and `rs_rtl::tuner::R82xx` are both `pub` and constructible, but `RtlSdr` hands out neither, and PPM's sample-rate half is computed inside `set_sample_rate` from the private `rtl_xtal_freq` (`rtlsdr.rs:557-563`) while retuning goes through the private `R82xx` built with the private `tuner_xtal_freq` — so driving them downstream means shadowing rs-rtl's own state and losing to it on the next retune. The right change is librtlsdr-shaped `set_freq_correction()` / `set_direct_sampling()` methods on `RtlSdr`. **Superseded — the decision is now to vendor both native drivers rather than depend on them**, after `hackrf-nusb` 0.3.0 was found to close its whole stream on a *single* errored transfer (`checked_completion` → `completion.status?` → `close()`, no counting and no `Cancelled` exemption), i.e. the same class of bug as rs-rtl's and worse. Two drivers, two divergent, both-wrong transfer-error policies is the actual defect; one shared transport with librtlsdr's policy is the fix, and it also makes these two settings ours to add. Plan: `PLAN-NATIVE-DRIVERS.md` |
| Native drivers taken in-tree (post-M5) | **Both native backends now own their radio driver outright**, one crate each: `crates/device-rtlsdr/src/driver/` (RTL2832U + R82xx) and `crates/device-hackrf/src/driver/` (HackRF), over the shared `crates/usb-stream` bulk transport (both directions). Historically the code started from `rs-rtl` 0.4.2 and `hackrf-nusb` 0.3.0 and was rebuilt on that transport; it is no longer a tracked fork of either, which is why "always newest versions" does not apply to it. Layering inside each backend is `driver` (radio, no wire types, no arbitration) → `convert` (the radio's sample table) → `caps` (pure wire translation) → the `SdrDevice` implementation over `crates/device`'s shared capture machinery. The error policy is librtlsdr's (`src/librtlsdr.c:3814`): a cancellation never counts, only genuine errors do, the threshold is the queue depth, any success clears it — plus rs-rtl's better-than-librtlsdr habit of resubmitting an errored transfer instead of retiring it, and a stop flag so a cancellation we asked for is a clean exit rather than a fault. Recovery is two-tier (`PLAN-NATIVE-DRIVERS.md` §2.2): the shared capture supervisor in `crates/device` restarts the stream in place (~3 ms measured) under a pure `RestartPolicy`, and only a restart that runs out of attempts reaches the engine's existing destructive fault path (~9 s). `device-soapy` deliberately keeps tier 2 only: librtlsdr *inside* SoapySDR already absorbs transient stalls, so an error surfacing through Soapy means that driver gave up. `device-virtual` gets none of it — no transport that can stall. Conversion moved to the device edge, where the two radios genuinely differ (HackRF cs8 two's-complement; RTL2832U unsigned around a measured 127.4 offset). Dropped rather than ported: rs-rtl's `StreamControl` in-thread retunes (fire-and-forget, so a rejected retune looked applied) and hackrf-nusb's `MaybeFuture`/async/wasm layers. Gained: `set_freq_correction` (PPM) on the RTL driver, a silent-stall detector on both backends, and the HackRF's transmit path, which upstream had and the first cut dropped — restored because a driver that omits half its hardware has to be rewritten when TX is built |
| Shared device machinery, not per-backend copies | The first cut of the native backends left each one owning its capture loop, and the two copies were ~105 identical lines that had already diverged on a bug. The rule now: **anything a second radio would have to write again lives in `crates/device`.** That is `Duplex`/`DuplexState` (§6 — the half-duplex rule, pure and unit-tested, replacing the HackRF driver's private `claim`/`select`), `Capture` + `CaptureRadio`/`CaptureStream` (the thread, the tier-1 restart supervisor, the silent-stall detector, block splitting, stop-and-join teardown), `LutConverter` (the 256-entry lookup plus the odd-length carry — only the *table* is a radio's own), `Worker` (the thread/stop-flag plumbing all four backends repeated), `RestartPolicy` and `lock`. The bulk-**OUT** queue moved down to `crates/usb-stream` beside bulk-IN for the same reason, leaving `device-hackrf` only the firmware's burst-end marker; `test-util` there exports the scripted endpoint so a backend's own burst tests use the transport's mock rather than a second copy. `crates/device` stays I/O-free by default — the `CaptureStream` impl for the USB transport is behind its `usb` feature, so a Soapy-only or virtual-only build compiles no USB stack. Two bugs the split fixed by construction: `rx_stop` used to switch the transceiver off unconditionally, which would silence a live transmit burst (now each direction releases only its own claim, and a `Capture` that is not running holds no radio at all), and a stop racing a tier-1 restart could leave the radio armed with no thread (now the control thread disarms *after* the join) |
