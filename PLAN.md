...
│   ├── usb-stream/     # bulk-IN transport for the native backends: transfer queue,
│   │                   # USB error policy, mock seam. Bytes only, no device knowledge.
│   ├── rtl-driver/     # vendored RTL2832U + R82xx driver, on usb-stream (§17).
│   ├── hackrf-driver/  # vendored HackRF driver, on usb-stream (§17).
...
| Single-maintainer native-driver crates | resolved by vendoring: `crates/rtl-driver` (from `rs-rtl` 0.4.2, sha `32c76e10c5b0c852cdc2550c5368b2fc0af8c611`) and `crates/hackrf-driver` (from `hackrf-nusb` 0.3.0), both rebuilt on `crates/usb-stream`. The trigger was not maintenance but correctness — each hand-rolled its own USB transfer-error policy and both were wrong (§18) — and the fork points are pinned deliberately rather than tracked, which is this project's stated exception to "always newest versions" |
