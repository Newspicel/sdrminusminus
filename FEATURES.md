...
- **[shipped]** Soft-decision BPTC measured end-to-end on the phase-0 DMR chain — the same C4FM -> channel filter -> `Fsk4Demod` stream decoded hard and soft (Chase-2 `Bptc196::decode_soft` on `fsk4::soft_bits`), with post-FEC BER over accepted frames, FER and undetected-error rate committed as separate curves: **1.60 dB gain at post-FEC BER 1e-3** (16.51 dB -> 14.91 dB) and FER collapsed 43% -> 8.6% at 16 dB, guarded as the phase-1 regression gate (`crates/channels/tests/dmr_soft_gain.rs`, `baselines/dmr/dmr_bptc_*.json`)
