...
- **[shipped]** That scope is keyed on the **patch node**, not the engine channel id: each stored row carries the node its channel belonged to, resolved against the active workspace's own binding as it was written. Engine ids are allocated per run and reused, so a scope built on them would hand a node another node's history after a restart. Rows written before the column, or before the binding caught up with a new channel, carry no node and are reached by the `device_set:channel` fallback — for the run that wrote them, which is the only run they can honestly be attributed in
