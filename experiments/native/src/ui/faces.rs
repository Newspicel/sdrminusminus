...
fn channel(store: Store, node: String) -> impl IntoView {
    ...
    let audio_of = move |pick: fn(&sdrmm_wire::audio::AudioProcessing) -> f64| {
        Signal::derive(move || {
            channel
                .get()
                .map_or(0.0, |channel| pick(&channel.settings.audio))
        })
    };
    let audio_flag = move |pick: fn(&sdrmm_wire::audio::AudioProcessing) -> bool| {
        Signal::derive(move || {
            channel
                .get()
                .is_some_and(|channel| pick(&channel.settings.audio))
        })
    };

    let blanker_on = audio_flag(|audio| audio.blanker.enabled);
    let blanker = audio_of(|audio| f64::from(audio.blanker.threshold));
    let click_on = audio_flag(|audio| audio.click_removal.enabled);
    let click = audio_of(|audio| f64::from(audio.click_removal.threshold));
    let denoise_on = audio_flag(|audio| audio.denoise.enabled);
    let denoise = audio_of(|audio| f64::from(audio.denoise.strength));
    let passband_on = audio_flag(|audio| audio.filter.enabled);
    let passband_read = move || {
        channel.get().map_or_else(
            || String::from("300 - 3000 Hz"),
            |channel| {
                let filter = &channel.settings.audio.filter;
                format!("{:.0} - {:.0} Hz", filter.low_hz, filter.high_hz)
            },
        )
    };
    ...
    let toggle_blanker = {
        ...
        move |on: bool| {
            write(Box::new(move |settings| {
                settings.audio.blanker.enabled = on
            }))
        }
    };
    let set_blanker = {
        ...
        move |value: f64| {
            write(Box::new(move |settings| {
                settings.audio.blanker.threshold = value as f32;
                settings.audio.blanker.enabled = true;
            }));
        }
    };
    let toggle_click = {
        ...
        move |on: bool| {
            write(Box::new(move |settings| {
                settings.audio.click_removal.enabled = on
            }))
        }
    };
    let set_click = {
        ...
        move |value: f64| {
            write(Box::new(move |settings| {
                settings.audio.click_removal.threshold = value as f32;
                settings.audio.click_removal.enabled = true;
            }));
        }
    };
    let toggle_denoise = {
        ...
        move |on: bool| {
            write(Box::new(move |settings| {
                settings.audio.denoise.enabled = on
            }))
        }
    };
    let set_denoise = {
        ...
        move |value: f64| {
            write(Box::new(move |settings| {
                settings.audio.denoise.strength = value as f32;
                settings.audio.denoise.enabled = true;
            }));
        }
    };
    let toggle_passband = {
        let write = write.clone();
        move |on: bool| write(Box::new(move |settings| settings.audio.filter.enabled = on))
    };

    let agc = Signal::derive(move || {
        channel
            .get()
            .map_or(AudioAgcMode::Off, |channel| channel.settings.audio.agc)
    });
    let pick_agc = {
        let write = write.clone();
        move |mode: AudioAgcMode| write(Box::new(move |settings| settings.audio.agc = mode))
    };
    let auto_notch = Signal::derive(move || {
        channel
            .get()
            .is_some_and(|channel| channel.settings.audio.auto_notch)
    });
    let toggle_notch = {
        let write = write.clone();
        move |on: bool| write(Box::new(move |settings| settings.audio.auto_notch = on))
    };
    ...
    view! {
        column(class = "face") {
            ...
            {row_field("Blanker", gated_slide(
                blanker_on,
                toggle_blanker,
                blanker,
                MIN_BLANKER_THRESHOLD.into(),
                MAX_BLANKER_THRESHOLD.into(),
                |value| format!("{value:.1}x"),
                set_blanker,
            ))}
            {row_field("De-click", gated_slide(
                click_on,
                toggle_click,
                click,
                MIN_CLICK_THRESHOLD.into(),
                MAX_CLICK_THRESHOLD.into(),
                |value| format!("{value:.1}x"),
                set_click,
            ))}
            {row_field("Denoise", gated_slide(
                denoise_on,
                toggle_denoise,
                denoise,
                0.0,
            ...
            {row_field("Passband", view! {
                row(class = "field__body") {
                    {check(passband_on, toggle_passband)}
                    text(class = "mono hint") {{passband_read}}
                }
            })}
        }
    }
